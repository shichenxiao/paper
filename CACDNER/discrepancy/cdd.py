...
class CDD(object):
    ...
    def forward(self, source, target, nums_S, nums_T):
        ...
        for i in range(self.num_layers):
            if i==0:
                cur_source = torch.reshape(source[i],(-1,128)) #[12,64]
                cur_target = torch.reshape(target[i],(-1,128)) #[12,64]
            elif i==1:
                cur_source = torch.reshape(source[i], (-1, 16))
                cur_target = torch.reshape(target[i], (-1, 16))
            dist = {}
            dist['ss'] = self.compute_paired_dist(cur_source, cur_source) #计算源源距离
            dist['tt'] = self.compute_paired_dist(cur_target, cur_target) #计算目标目标距离
            dist['st'] = self.compute_paired_dist(cur_source, cur_target)  #计算源-目标距离

            # dist['ss'] = self.split_classwise(dist['ss'], nums_S) # 计算class_aware源域类间距离
            # dist['tt'] = self.split_classwise(dist['tt'], nums_T) #计算class_aware目标域类间距离
            dist_layers += [dist]
