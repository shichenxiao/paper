...
class Bert_BiLSTM_CRF(nn.Module):
    def __init__(self, tag_to_ix, hidden_dim=768):
        ...
        self.lstm = nn.LSTM(bidirectional=True, num_layers=2, input_size=768, hidden_size=128,
                            batch_first=True)
        ...
        self.fc = nn.Linear(256, self.tagset_size)
    ...
    def neg_log_likelihood(self, sentence, tags):
        enc,feats = self._get_lstm_features(sentence)  #[batch_size, max_len, 16]
    ...
    def _get_lstm_features(self, sentence):
        ...
        enc, _ = self.lstm(embeds)
        lstm_feats = self.fc(enc)
        return enc,lstm_feats  # [8, 75, 16]

    def forward(self, sentence):  # dont confuse this with _forward_alg above.
        # Get the emission scores from the BiLSTM
        enc,lstm_feats = self._get_lstm_features(sentence)  # [8, 180,768]

        # Find the best path, given the features.
        score, tag_seq = self._viterbi_decode(lstm_feats)
        return enc,score, tag_seq
