...
class LossFct(object):
    def __init__(self, type='cross_entropy', args=None, num_labels=None,ignore_index=None, **kwargs):
        super(LossFct).__init__()
        self.type = type
        self.add_polynominal = args.add_polynominal
        self.num_labels = num_labels
        self.epsilon = 1.0

        if self.type == 'cross_entropy':
            self.loss_fct = nn.CrossEntropyLoss(ignore_index=ignore_index,**kwargs)
        if self.type == 'focal':
            self.loss_fct = FocalLoss(ignore_index=ignore_index,**kwargs)
        if self.type == 'dice':
            self.loss_fct = DiceLoss(**kwargs)

    def __call__(self, logits, labels):
        loss = self.loss_fct(logits, labels)

        if self.add_polynominal and self.type == 'focal':
            p = torch.nn.functional.sigmoid(logits)
            labels = torch.nn.functional.one_hot(labels, self.num_labels)
            labels = torch.tensor(labels, dtype=torch.float32)
            poly1 = labels * p + (1 - labels) * (1 - p)
            loss = loss + torch.mean(self.epsilon * torch.pow(1 - poly1, 2 + 1), dim=-1)

        if self.add_polynominal and self.type == 'cross_entropy':
            poly = torch.sum(F.one_hot(labels, self.num_labels).float() * F.softmax(logits), dim=-1)
