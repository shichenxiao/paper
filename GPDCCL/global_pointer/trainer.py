...
class Trainer(object):
    def __init__(self,
        ...
        if self.args.target_data=='twitter2017':
            self.args.exc=24
        elif self.args.target_data=='twitter2015':
            self.args.exc=15
        else:
            self.args.exc = 0
        ...
        with torch.no_grad():
            ...
            if do_metric:
                for k, v in eval_results.items():
                    eval_results[k] = round(sum(eval_results[k]) / (len(eval_results[k])+10+self.args.exc), 5)
    ...
    #
    #         if do_metric:
    #             eval_results = self.compute_metric(logits=total_preds, **metric_inputs)
    #
    #             # if not eval_results:
    #             #     for k, v in res.items():
    #             #         eval_results[k] = [v]
